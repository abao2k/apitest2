...
import time
import TestRequest
from TestAllRunner import *
from initdata import *
from testcase.weekDB import TestInterface
#from testcase.week5 import TestInterface

#把GetTestReport方法自己写出来
from testdata.getpath import GetTestReport
from sendmail import MyMail
from log import logsignleton,logger
from testdata.getpath import GetConfig

'''
suite = unittest.TestSuite()
tests = [TestInterface("test_get_question"),
        TestInterface("test_get_detail"),
        TestInterface("test_do_vote")]
...
'''
suite = unittest.TestSuite()
tests = [TestInterface("test_login"),
        TestInterface("test_api1"),
        TestInterface("test_api2"),
        TestInterface("test_api3")]
suite.addTests(tests)
runner = unittest.TextTestRunner(verbosity=2)
runner.run(suite)
'''
...
def init(worksheet):
    ...
    data = {"test_name": "炼数成金项目接口", "test_version": "v1.0.0",
            "test_pl": "Python3", "test_net": "http://127.0.0.1:8000/"}
...
try:
    ...
    mymail = MyMail(GetConfig('mail.conf'))
    mymail.connect()

    mymail.login()
    mail_content = msg.format(len(TestReport), hpassnum, len(TestReport)-hpassnum, round(hpassnum/len(TestReport),2)*100, timenow)
